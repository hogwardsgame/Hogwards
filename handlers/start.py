...
def main_menu_keyboard(user_id: int) -> ReplyKeyboardMarkup:
    # Бот теперь только для админки — вся игра в Mini App.
    # Обычные игроки сюда не попадают (им показывается кнопка "Открыть игру").
    if user_id in ADMIN_USER_IDS:
        buttons = [
            [KeyboardButton(t(user_id, "btn_admin_panel"))],
    ...
    # Не-админ: пустое меню (игра в приложении)
    return ReplyKeyboardMarkup([[KeyboardButton("🪄 Игра в приложении")]], resize_keyboard=True)


def other_menu_keyboard(user_id: int) -> ReplyKeyboardMarkup:
    # Оставлено для совместимости — только админка
