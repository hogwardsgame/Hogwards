...
from database import get_user, get_user_spells, user_exists
...
async def show_profile(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    async with pool.acquire() as conn:
        ...
        spells_count = await conn.fetchval(
            "SELECT COUNT(*) FROM user_spells WHERE user_id = $1", user_id
        )
...
async def handle_profile_button(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    profile_btn = t(user_id, "btn_profile")
    if update.message.text == profile_btn:
