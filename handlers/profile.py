...
def _combat_overview(user: dict, equipment_bonuses: dict) -> str:
    ...
    return (
        f"\n━━━━━━━━━━━━━━━━━━━━\n"
        f"📊 *Характеристики*\n"
        f"⚔️ Урон:     {user['attack']}  _(база {base_attack} + экип. {equipment_bonuses.get('attack',0)})_\n"
        f"🛡️ Защита:   {user['defense']}  _(база {base_defense} + экип. {equipment_bonuses.get('defense',0)})_\n"
        f"⚡ Скорость: {user['speed']}  _(база {base_speed} + экип. {equipment_bonuses.get('speed',0)})_\n"
        f"🍀 Удача:    {user['luck']}  _(база {base_luck} + экип. {equipment_bonuses.get('luck',0)})_\n"
        f"💧 Мана макс:{user['max_mana']}  _(база {base_mana} + экип. {equipment_bonuses.get('max_mana',0)})_"
    )
...
async def show_profile(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    equipment = f"\n━━━━━━━━━━━━━━━━━━━━\n🎒 *Надетое снаряжение*\n{equipment_text}"
    # Telegram user_id строка внизу профиля
    id_line   = f"\n━━━━━━━━━━━━━━━━━━━━\n🆔 Твой ID: `{user_id}`"

    await update.message.reply_text(
        f"{header}\n{body}{combat}{equipment}{id_line}",
        parse_mode="Markdown"
    )
