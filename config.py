...
# ── ЭКОНОМИКА (полностью пересчитана) ──────────────────────────────────────────
# Стартовое золото = 0 (защита от мультиаккаунтов)
STARTER_GOLD = 0
STARTER_MANA = 50
STARTER_HP   = 100
...
# ── ПРОГРЕССИЯ XP (замедленная — долгосрочная прокачка) ────────────────────────
# Формула: XP_нужно = BASE * N * MULT^(N-1)
# Старая: BASE=500, MULT=1.15 — слишком быстро
# Новая:  BASE=1200, MULT=1.20 — несколько месяцев игры
